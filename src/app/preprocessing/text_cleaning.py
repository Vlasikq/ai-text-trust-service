...
Используется в inference-пайплайне (`pipeline.py`) и в сборке датасета
(`scripts/assemble_dataset.py`). Порядок: normalize_whitespace → strip_markdown
(сначала убираем \n, чтобы regex для **bold** не ломался на многострочных
фрагментах).
"""
